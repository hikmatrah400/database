...
const InputList = ({
  ...
  const SubmitForm = (e) => {
    ...
    const filterData = Data[0].filter((elm) => elm.id === IndexNum[0]);
    ...
    const updateCode = filterCode.filter((elm) => elm.id !== IndexNum[0]);

    const filterItem = Data[0].filter((elm) => elm.item === item);
    const getItemIndex = filterItem.filter((elm) => elm.id !== IndexNum[0]);
